...
def main():
    ...
    entity_resolver = entity_resolver[['Entity Name','Canonical Name (Syntactic)']]
...
#    contracts['supplier'] = contracts['supplier'].map(lambda x: str(x).replace(')', ''))
#    contracts['supplier'] = contracts['supplier'].map(lambda x: str(x).replace('(', ''))
    df_new = pd.DataFrame({'Entity Name':[], 'Canonical Name (Syntactic)':[]})
    df_new['Entity Name']=entity_resolver['Canonical Name (Syntactic)']
   # df_new['Canonical Name (Syntactic)'] = entity_resolver['Canonical Name (Syntactic)']
    ...
    entities = pd.DataFrame(entity_resolver.groupby('Entity Name')['Canonical Name (Syntactic)'].apply(get_unique))


    entity_dict = entities.to_dict()['Canonical Name (Syntactic)']
...
#    contracts['resolved_supplier'] = contracts['resolved_supplier'].map(lambda x: str(x).replace('"', ''))
   ...
   # contracts['resolved_supplier']=contracts['resolved_supplier'].str.replace('\s', '')
    ...
    print "Number of entities before resolution: ", contracts['orig_supplier_name'].nunique()
